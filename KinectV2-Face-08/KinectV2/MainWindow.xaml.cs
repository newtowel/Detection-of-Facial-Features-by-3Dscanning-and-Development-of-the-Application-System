...
namespace KinectV2
{
    ...
    public partial class MainWindow : Window
    {
        ...
        void DrawFaceFrameResult(int faceIndex, FaceFrameResult faceResult, DrawingContext drawingContext)
        {
            ...
            {
                Console.WriteLine("x座標 : {0}, y座標 : {1}", pointF.X, pointF.Y);
            ...
            Rect rect = new Rect(box.Left, box.Top, Math.Abs(width), Math.Abs(height));
            drawingContext.DrawRectangle(null, drawingPen, rect);
            String drawingText = String.Empty;

            Console.WriteLine("1ターン終了\n\n");

            StreamWriter sw = new StreamWriter(@"C:\Users\2018-31\Desktop\1\2.txt", false, Encoding.UTF8);
            foreach (PointF pF in facePoints.Values)
            {
                sw.WriteLine(pF.X.ToString());
                sw.WriteLine(pF.Y.ToString());
